...
#
# It is desirable that all of the expected TypeSlice's minimized program gets compiled, because TypeSlice
# should produce independently-compilable programs.

# when this script is invoked from TypeSlice CI,
...
for target in $issue_ids; do
   ...
   if [ "$target" = "jdk-8319461" ]; then continue; fi
    ...
    # check if any directory exists inside output. If no directory there, TypeSlice failed on the input target
    ...
    if [ "$target" = "na-97" ]; then
      javac -classpath "$TypeSlice/src/test/resources/shared/checker-qual-3.42.0.jar" $(find . -name "*.java") --patch-module java.base=jdk/src
    else
      javac -classpath "$TypeSlice/src/test/resources/shared/checker-qual-3.42.0.jar" $(find . -name "*.java")
    fi
    javac_status=$?
